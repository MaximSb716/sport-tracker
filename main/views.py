...
def catalog(request):
    # Инициализация формы фильтрации с данными из GET запроса
    ...
    if filter_form.is_valid():
        ...
        type_filter = filter_form.cleaned_data.get('type_of_inventory')
        ...
        # Фильтр по типу состояния
        if type_filter:
            categories = categories.filter(type_of_inventory=type_filter)
        
        # Фильтр по минимальному количеству
        if min_quantity is not None:
            categories = categories.filter(questions_number__gte=min_quantity)
        
        # Фильтр по максимальному количеству
        if max_quantity is not None:
            categories = categories.filter(questions_number__lte=max_quantity)
        ...
        if sort_by:
            if sort_by == 'name_asc':
                ...
                categories = categories.order_by('questions_number')
            elif sort_by == 'quantity_desc':
                categories = categories.order_by('-questions_number')
        else:
            # Сортировка по умолчанию (по ID, новые первыми)
            categories = categories.order_by('-id')
    else:
        # Если форма невалидна, используем сортировку по умолчанию
        categories = categories.order_by('-id')
    ...
    # Формирование данных для отображения с безопасной проверкой директорий
    data = []
    for category in categories:
        ...
        if os.path.exists(directory) and os.path.isdir(directory):
            try:
                ...
                if files:
                    url_to_header = f"/uploads/inventorys/admin/{category.id}/{files[0]}"
            except (OSError, PermissionError):
                # Если нет доступа к директории, оставляем пустую строку
                url_to_header = ""
